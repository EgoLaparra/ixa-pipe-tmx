...
public class TlinkCategorizationDCTTimex {

	
	public TlinkCategorizationDCTTimex() {
	...
	public static void main(String[] args) {
		...
		try {
			if (args[0].equals("train")) {
				...
				dataset.loadTlinksFromTempeval2(args[1] + "/tlinks-dct-timex.tab");
				dataset.loadTlinkFeaturesFromNAF(args[2]);
				dataset.createTlinkCategorizationCRFTrain(DCT.class, Timex.class);
				System.out.println("\t...training model");
				...
				dataset.loadEventAttributesFromTempeval2(args[1] + "/event-attributes.tab");
				dataset.loadTimexAttributesFromTempeval2(args[1] + "/timex-attributes.tab");
				dataset.loadUnknownTlinksFromTempeval2(args[1] + "/tlinks-dct-timex.tab", args[2]);
				//dataset.createUnknownTlinks(args[2]);
				dataset.loadTlinkFeaturesFromNAF(args[2]);
				dataset.createTlinkCategorizationCRFTag(DCT.class, Timex.class);
				System.out.println("\t...tagging");
				dataset.crfY = client.tag(dataset.crfX, args[3]);
				dataset.tagTlinks();
				System.out.println("\t...printing output");
				dataset.printTempEval2Tlinks(args[4], DCT.class, Timex.class);
			}
